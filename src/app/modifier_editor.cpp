...
void EditorUi::draw_modifier_ui(){
    ...
    ImGui::TextWrapped("Masks stay in cloud-local metres. Full hard cuts remain empty; protection suppresses detail without saving density snapshots.");
    ...
    if(ImGui::SmallButton("Solo")){auto scene=session.document().scene();for(auto& l:scene.finish_stack.layers)l.enabled=l.id==finish_selected_;apply(std::move(scene));}
    ...
    if(ImGui::BeginCombo("Target",target.c_str())){
        if(ImGui::Selectable("Whole cloud",layer.target_kind==FinishTargetKind::object)){layer.target_kind=FinishTargetKind::object;layer.target_id=session.document().scene().frozen->id;apply(scene_with_finish_command(session.document().scene(),{FinishCommandKind::replace,layer.id,layer}));}
        const auto fields=session.document().scene().frozen->fields;for(const auto& field:fields){const auto name="Field "+std::to_string(field.development_id);if(ImGui::Selectable(name.c_str(),layer.target_kind==FinishTargetKind::field&&layer.target_id==field.development_id)){layer.target_kind=FinishTargetKind::field;layer.target_id=field.development_id;apply(scene_with_finish_command(session.document().scene(),{FinishCommandKind::replace,layer.id,layer}));}}
        ImGui::EndCombo();
    }
}
