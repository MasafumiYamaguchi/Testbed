...
std::uint64_t sun_cache_key(const Scene& scene,std::array<std::uint32_t,3> extent,unsigned resolution,unsigned steps){
    ...
    for(double v:{scene.sun.direction_to_light.x,scene.sun.direction_to_light.y,scene.sun.direction_to_light.z,scene.cloud.optics.extinction_scale})add(std::bit_cast<std::uint64_t>(v));
...
double sun_optical_depth(const Scene& scene,const GridLayout& grid,std::span<const float> density,Vec3 origin,unsigned steps){
    ...
    const auto& r=scene.cloud;auto linear=r.transform;linear.translation={0,0,0};const auto direction=world_to_local(linear,scene.sun.direction_to_light);
    const auto interval=intersect_bounds(origin,direction,r.envelope,0,std::numeric_limits<double>::max());if(!interval)return 0;
