...
const DevelopedCloud* editable_developed_source(const Scene& scene){if(scene.top_lobes)return &scene.top_lobes->trunk;return scene.developed?&*scene.developed:nullptr;}
...
bool scene_has_active_top_lobes(const Scene& scene){return scene.top_lobes&&TopLobeEvaluationPlan(*scene.top_lobes).gpu_params().mask.x!=0;}
bool scene_density_requires_direct(const Scene& scene){return scene_has_multiple_developments(scene)||scene_has_active_top_lobes(scene);}
...
Scene scene_with_developed_command(Scene scene,const DevelopedCommand& command) {
    if(scene.top_lobes)return scene_with_top_lobe_command(std::move(scene),TopLobeTrunkCommand{command});
    if(!scene.developed)throw std::invalid_argument("Scene has no developed source");
...
SceneDensityEvaluator::SceneDensityEvaluator(const Scene& scene):recipe_(scene.cloud) {
    require_valid(scene);
    if(scene_has_active_top_lobes(scene))top_.emplace(*scene.top_lobes);
...
double SceneDensityEvaluator::at(Vec3 local)const{return top_?top_->at(local):developed_?developed_->at(local):single_->at(local);}
double SceneDensityEvaluator::maximum()const{return top_?top_->maximum():developed_?developed_->maximum():single_->maximum();}
Bounds SceneDensityEvaluator::local_support()const{return top_?top_->local_support():developed_?developed_->local_support():single_->local_support();}
Bounds SceneDensityEvaluator::world_support()const{return top_?top_->world_support():developed_?developed_->world_support():single_->world_support();}
GpuTopLobeParams SceneDensityEvaluator::gpu_params()const {
    if(top_)return top_->gpu_params();
...
GpuTopLobeParams gpu_scene_density_params(const Scene& scene){return SceneDensityEvaluator(scene).gpu_params();}
}
