...
void RevisionQueue::request(DensityJob job){
    ...
    if((published_hash_&&*published_hash_==latest_hash_)||(running_&&running_->input_hash==latest_hash_)){if(pending_)++coalesced;pending_.reset();return;}
...
bool RevisionQueue::finish(std::uint64_t revision,std::uint64_t hash,bool successful){
    ...
    running_.reset();if(successful&&hash==latest_hash_){published_hash_=hash;pending_.reset();++published;return true;}
