...
int main(){try {
    ...
    check(scene.schema_version==8&&scene.algorithm_version==3,"Wrong source/evaluation version");
    const SceneDensityEvaluator full(scene);const TopLobeEvaluationPlan expected(*scene.top_lobes);const auto gpu=full.gpu_params(),reference_gpu=expected.gpu_params();
    ...
    check(std::memcmp(&gpu,&reference_gpu,sizeof(gpu))==0,"Scene wrapper omitted hierarchy mask/GPU data");
