...
int main(){try {
    ...
    check(encoded.at("schema_version")==11&&encoded.at("cloud").size()==2&&encoded.at("cloud").at("kind")=="cumulonimbus","Prefab not encoded as source-only schema7 object");
    ...
    for(unsigned version=1;version<=4;++version) {
        ...
        check(migrated==old&&migrated.schema_version==11&&!migrated.cumulonimbus,"Legacy Custom Cloud migration changed values or inferred prefab");
    }
