...
int main(){try {
    ...
    check(encoded.at("schema_version")==5&&encoded.at("cloud").size()==2&&encoded.at("cloud").at("kind")=="cumulonimbus","Prefab not encoded as source-only schema5 object");
    ...
    check(parse_scene_json(custom_json.dump())==custom,"Custom schema5 round-trip failed");
    ...
    for(unsigned version=1;version<=4;++version) {
        ...
        auto legacy=Json::parse(scene_json(old));legacy["schema_version"]=version;
        ...
        check(migrated==old&&migrated.schema_version==5&&!migrated.cumulonimbus,"Legacy Custom Cloud migration changed values or inferred prefab");
    }
    ...
    std::cout<<"Cumulonimbus Scene schema5: source-only round-trip; uint64 values; schemas1..4 migration; stale snapshot rejection; EditorSession Undo/add/remove/conversion; atomic failure preservation passed\n";
    return 0;
