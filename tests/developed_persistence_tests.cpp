...
int main(){try {
    ...
    check(scene.schema_version==11&&scene.algorithm_version==3&&scene_density_requires_direct(scene),"Two developments lack schema7/direct evaluation contract");
    ...
    check(packet.field.cloud.fields.settings.x==2&&std::memcmp(&packet.field.cloud.fields,&expected_packet,sizeof(expected_packet))==0,"GPU packet did not contain both developments");
