...
// Conservative float mask/stack roundoff, including protection's effect on
// warp displacement, erosion distance and medium density modulation.
double ellipsoid_mask_error_bound(const EllipsoidMask&);
struct ModifierPrecisionBudget {
    std::array<double,2> density_error{},detail_error{},density_gain{1,1};
};
ModifierPrecisionBudget finish_precision_budget(const FinishStack&,Id object_id,std::span<const Id> fields,double sample_position_error=0);
double finish_density_error_bound(const FrozenCloudState&,const FinishStack&);
